...
router.post('/import-latest', async (_req, res) => {
  let client;
  try {
    const resp = await fetch(TWSE_API_URL);
    if (!resp.ok) {
      return res.status(502).json({ success: false, error: `TWSE API 狀態碼 ${resp.status}` });
    }

    const rawList = await resp.json();
    if (!Array.isArray(rawList) || !rawList.length) {
      return res.status(502).json({ success: false, error: 'TWSE 回傳資料為空' });
    }
